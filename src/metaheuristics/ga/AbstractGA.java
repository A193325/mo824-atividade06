...
public abstract class AbstractGA<G extends Number, F> {
	...
	public Double maxMutationRate;
	public Boolean uniformCross;
	 ...
	 */
	protected abstract Solution<F> fitness(Chromosome chromosome);
	 ...
	 *            The mutation rate.
	 */
	public AbstractGA(Evaluator<F> objFunction, Integer generations, Integer popSize, Double mutationRate, Double maxMutationRate, Boolean uniformCross) {
	...
	public Solution<F> solve() {
		...
		for (int g = 1; g <= generations; g++) {
			...
			Population mutants = mutate(offsprings, mutRate);
	...
	protected Population initializePopulation() {
		System.out.println("start initialize Population");
	...
	protected Population selectParents(Population population) {
		...
		while (parents.size() < popSize) {
			...
			if (fitness(parent1).cost > fitness(parent2).cost) {
				parents.add(parent1);
			} else {
				parents.add(parent2);
			}
	 ...
	 */
	protected Population mutate(Population offsprings, Double mutRate) {
